...
# ----------------------------------------------------------------------------------------------------------------------
class CKA_Monkey_folds(CKA_Monkey):
    """
    ...
    def calculation_CKA_Similarity_folds(self, kernel='linear', **kwargs) -> dict:
    ...
    def collect_CKA_Similarity_folds(self, kernel='linear', **kwargs) -> dict:
...
class CKA_Human_folds(CKA_Human):
...
# ----------------------------------------------------------------------------------------------------------------------
class CKA_Monkey_Comparison(CKA_Monkey_folds, CKA_Monkey):
    """
        ...
    """
    ...
    def __call__(self, kernel, **kwargs):
        ...
        for idx, (root, model) in enumerate(self.roots_and_models):
            ...
            if 'fold' in root:
                ...
                CKA_Monkey_folds.__init__(self, root=self.roots_and_models[idx][0], **kwargs)     # save the comparison results in the fisrt folder
                ...
                CKA_Monkey.__init__(self, root=self.roots_and_models[idx][0], **kwargs)     # save the comparison results in the fisrt folder
                _, self.layers, self.neurons, _ = utils_.get_layers_and_units(self.roots_and_models[idx][1], 'act')
...
class CKA_Human_Comparison(CKA_Human_folds, CKA_Human):
    """
        ...
    """
    ...
    def __call__(self, kernel, **kwargs):
        ...
        for idx, (root, model) in enumerate(self.roots_and_models):
            ...
            if 'fold' in root:
                ...
                CKA_Human_folds.__init__(self, root=self.roots_and_models[idx][0], **kwargs)     # save the comparison results in the fisrt folder
                _, self.layers, self.neurons, _ = utils_.get_layers_and_units(self.roots_and_models[idx][1], 'act')
                ...
                CKA_Human.__init__(self, root=self.roots_and_models[idx][0], **kwargs)     # save the comparison results in the fisrt folder
                _, self.layers, self.neurons, _ = utils_.get_layers_and_units(self.roots_and_models[idx][1], 'act')
...
#         
#         CKA_Monkey(root=root, layers=layers, neurons=neurons)()
# 
#         CKA_human = CKA_Human(root=root, layers=layers, neurons=neurons)
#         
#         for used_unit_type in used_unit_types:
...
# =============================================================================
    ...
    #CKA_Monkey_folds(num_folds=5, root=root, layers=layers, neurons=neurons)()

    CKA_human_f = CKA_Human_folds(num_folds=5, root=root, layers=layers, neurons=neurons)
...
#     CKA_Monkey_Comparison(layers, roots_models, primate_config='Monkey')(kernel='linear')
#     #for threshold in [1.0, 10.0]:
#     #    CKA_Monkey_Comparison(roots_models, primate_config='Monkey')(kernel='rbf', threshold=threshold)
#     
#     #for _ in used_unit_types:
#     #    CKA_Human_Comparison(roots_models, primate_config=f'Human/linear/{_}/50')(kernel='linear')
# =============================================================================
